    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md print:static print:max-h-none print:w-full print:max-w-none print:translate-x-0 print:translate-y-0 print:overflow-visible print:ring-0">
        <DialogHeader className="flex-row items-start justify-between">
          <div>
            <DialogTitle>
              {schoolName ? `${schoolName} — ` : ""}Fee statement
            </DialogTitle>
            <DialogDescription>
              {studentName} · Monthly fee {money(monthlyFee, currency)} · Lifetime
              collected {money(lifetimeCollected, currency)}
            </DialogDescription>
          </div>
          <Button
            variant="outline"
                ...
                {sorted.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td className="py-2 pr-2 text-foreground">
                      {money(p.amountPaid, currency)}
                      {p.amountDue !== p.amountPaid && p.status !== "social_case" && (
                        <span className="text-muted-foreground"> / {money(p.amountDue, currency)}</span>
                      )}
                    </td>
